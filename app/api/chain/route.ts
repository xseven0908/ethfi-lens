...
async function readNetwork(network:(typeof networks)[number]){const [shares,rate,pendingShares,tokenSupply,blockHex]=await Promise.all([call(network.rpc,SETHFI,"0x18160ddd"),call(network.rpc,ACCOUNTANT,"0x679aefce"),call(network.rpc,SETHFI,balanceOfData(DELAYED)),call(network.rpc,network.ethfi,"0x18160ddd"),rpc(network.rpc,"eth_blockNumber",[])]);return {name:network.name,shares,rate,staked:shares*rate,pending:pendingShares*rate,tokenSupply,blockNumber:Number(BigInt(blockHex)),ok:true}}

export async function GET(){
  const results=await Promise.allSettled(networks.map(readNetwork));
  const chains=results.map((result,index)=>result.status==="fulfilled"?result.value:{name:networks[index].name,shares:0,rate:0,staked:0,pending:0,tokenSupply:0,blockNumber:0,ok:false});
  const ethereum=chains.find(x=>x.name==="Ethereum"),mainnetSupply=ethereum?.tokenSupply||0,stale=chains.some(x=>!x.ok);
  return NextResponse.json({ethfiSupply:MAX_SUPPLY,mainnetSupply,supplyAdjustment:mainnetSupply?Math.max(0,MAX_SUPPLY-mainnetSupply):0,burned:0,burnStatus:"官方未启用代币销毁；回购所得分配给 sETHFI 持有人",staked:chains.reduce((sum,x)=>sum+x.staked,0),pending:chains.reduce((sum,x)=>sum+x.pending,0),pendingLabel:"解押合约可验证托管余额",chains,source:"Ethereum · Arbitrum · Base 公共 RPC",stale,updatedAt:new Date().toISOString()},{headers:{"Cache-Control":"public, max-age=20, s-maxage=20, stale-while-revalidate=60"}});
}
