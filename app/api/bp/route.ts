...
export async function GET(request:NextRequest){
  ...
  const [tickerResult,stakingResult,chartResult,depthResult,cgResult,fxResult,chainResult]=await Promise.allSettled([readTicker(),readStaking(),readChart(),readDepth(),readCoinGecko(),readCnyRate(),readChain()]);
  ...
  const bidDepth=bids.filter(x=>Number(x[0])>=mid*.99).reduce((sum,x)=>sum+Number(x[0])*Number(x[1]),0),askDepth=asks.filter(x=>Number(x[0])<=mid*1.01).reduce((sum,x)=>sum+Number(x[0])*Number(x[1]),0),chain=chainResult.status==="fulfilled"?chainResult.value:null;
  ...
  return NextResponse.json({
    ...
    stale:(currency==="cny"&&fx.stale)||!cg||rawChart.length<2||!chain||staked==null,failedSources:[!cg&&"CoinGecko",rawChart.length<2&&"Backpack K线",!chain&&"Solana RPC",staked==null&&"Backpack质押统计"].filter(Boolean),fxSource:currency==="cny"?fx.source:"USD",updatedAt:new Date().toISOString()
