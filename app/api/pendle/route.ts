...
const VERIFIED_CIRCULATING_SUPPLY=163_815_032;
const VERIFIED_TOTAL_SUPPLY=258_446_028;
...
async function readCoinPaprika(){return json("https://api.coinpaprika.com/v1/tickers/pendle-pendle",{},7000)}
...
export async function GET(request:NextRequest){
  ...
  const [binance,okx,bybit,binanceChart,okxChart,cgResult,paprikaResult,fxResult,spendleResult]=await Promise.allSettled([readBinance(),readOkx(),readBybit(),readBinanceChart(),readOkxChart(),readCoinGecko(),readCoinPaprika(),readCnyRate(),readSpendle()]);
  const venues:Venue[]=[];
  ...
  const paprika=paprikaResult.status==="fulfilled"?paprikaResult.value:null;
  ...
  const priceUsd=median(venues.map(item=>item.price)),paprikaPrice=Number(paprika?.quotes?.USD?.price??0),paprikaMarketCap=Number(paprika?.quotes?.USD?.market_cap??0);
  const paprikaCirculating=paprikaPrice>0&&paprikaMarketCap>0?paprikaMarketCap/paprikaPrice:0;
  const circulatingSupply=Number(cg?.circulating_supply)||paprikaCirculating||VERIFIED_CIRCULATING_SUPPLY,totalSupply=Number(cg?.total_supply)||Number(paprika?.total_supply)||VERIFIED_TOTAL_SUPPLY;
  const marketCapUsd=circulatingSupply*priceUsd,fdvUsd=totalSupply*priceUsd,athUsd=Number(cg?.ath)||Number(paprika?.quotes?.USD?.ath_price)||0;
  const supplySource=cg?"CoinGecko":paprika?"CoinPaprika":"最近校验供应量";
  ...
  const failedSources=[!cg&&!paprika&&"市场供应",rawChart.length<2&&"交易所 K 线",!spendle&&"Pendle sPENDLE API"].filter(Boolean);
  return NextResponse.json({
    ...
    circulatingSupply,totalSupply,ath:athUsd*rate,high24h:Math.max(...venues.map(item=>item.high24h))*rate,low24h:Math.min(...venues.map(item=>item.low24h))*rate,
    ...
    terminalInflationRate:2,vestingStatus:"团队与投资人代币已完成归属",currencyRate:rate,marketSource:venues.map(item=>item.name).join(" · "),supplySource,sPendleSource:"Pendle Core API /v1/spendle/data",fxSource:currency==="cny"?fx.source:"USD",failedSources,stale:(currency==="cny"&&fx.stale)||failedSources.length>0,updatedAt:new Date().toISOString()
