...
test("server-renders the Token Lens production shell", async () => {
  ...
  assert.match(html, /<title>Token Lens — ETHFI、BP 与 PENDLE 数据看板<\/title>/i);
...
test("keeps all three asset dashboards and production metadata wired", async () => {
  const [page, pendle, pendleApi, marketsApi, layout, packageJson] = await Promise.all([
  ...
  assert.match(page, /type AssetView = "ethfi" \| "bp" \| "pendle"/);
  ...
  assert.match(layout, /og-brand-v4\.png/);
